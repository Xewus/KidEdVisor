...
from .conftest import INVALID_TOKEN, ME_URL


@pytest.mark.smoke
class TestAccessToEndpoints:
    async def test_access_to_endpoint(self, http_client: TestClient):
        # no these methods
        assert (
...
class TestSelfManageParent:
    @pytest.mark.auth_parents
    def test_get_me(self, token_parent_1: tuple[TestClient, str]):
        http_client, token = token_parent_1
    ...
    def test_delete_me(
        self,
        token_parent_1: tuple[TestClient, str],
    ):
        http_client, token = token_parent_1
